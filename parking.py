...
# Función para establecer el número de autos
def set_cars():
    global num_autos
    num_autos = int(num_autos_entry.get())  # Actualizar el número de autos
    specs_label.config(text=f"Número de autos establecidos: {num_autos}")
    print(f"Autos disponibles para estacionar: {num_autos}")

# Función para buscar un espacio de estacionamiento y entrenar el modelo
def find_parking():
    global num_autos
    if num_autos <= 0:  # Verificar si ya no hay más autos para estacionar
        specs_label.config(text="No hay más autos por estacionar.")
        print("No hay más autos por estacionar.")
        return
    ...
    if reward == 1:  # Si encontró un espacio libre
        num_autos -= 1  # Reducir el número de autos disponibles
        ...
        car_specs += f"Autos restantes por estacionar: {num_autos}"
...
# Entrada para el número de autos
num_autos_label = tk.Label(root, text="Número de autos a estacionar:")
num_autos_label.grid(row=1, column=0, padx=5, pady=5)

num_autos_entry = tk.Entry(root)
num_autos_entry.grid(row=1, column=1, padx=5, pady=5)

set_cars_button = tk.Button(root, text="Establecer Autos", command=set_cars)
set_cars_button.grid(row=1, column=2, padx=5, pady=5)

# Botón para buscar un espacio de estacionamiento
evaluate_button = tk.Button(root, text="Buscar Estacionamiento", command=find_parking)
evaluate_button.grid(row=2, column=0, columnspan=3, pady=10)
